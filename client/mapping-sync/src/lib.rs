...
// This file is part of rococo-v1.
//
...
pub fn sync_one_level<Block: BlockT, C, B>(
	client: &C,
	substrate_backend: &B,
	rococo-v1_backend: &fc_db::Backend<Block>,
) -> Result<bool, String> where
	C: ProvideRuntimeApi<Block> + Send + Sync + HeaderBackend<Block> + BlockOf,
	C::Api: EthereumRuntimeRPCApi<Block>,
	B: sp_blockchain::HeaderBackend<Block> + sp_blockchain::Backend<Block>,
{
	let mut current_syncing_tips = rococo-v1_backend.meta().current_syncing_tips()?;
	...
	if current_syncing_tips.len() == 0 {
		...
		sync_genesis_block(client, rococo-v1_backend, &header)?;

		current_syncing_tips.push(header.hash());
		rococo-v1_backend.meta().write_current_syncing_tips(current_syncing_tips)?;
		...
		if let Some((syncing_tip, children)) = syncing_tip_and_children {
			...
			for child in children {
				...
				sync_block(rococo-v1_backend, &header)?;
	...
	B: sp_blockchain::HeaderBackend<Block> + sp_blockchain::Backend<Block>,
{
	...
	for _ in 0..limit {
		synced_any = synced_any || sync_one_level(client, substrate_backend, rococo-v1_backend)?;
	}
